		...
		<article>
				...
				<div id="style_poste">
						...
						if (!isset($_SESSION['pseudo']) || isset($_SESSION['pseudo'])) {
				           ...
				           $sqls= " SELECT * FROM tp_user LEFT JOIN tp_billet ON tp_user.id_user = tp_billet.id_user LEFT JOIN categorie ON tp_billet.id_categorie = categorie.id_categorie WHERE categorie.id_categorie = ".$_GET['sreemsvxy56Y63MIKyvnbsdnsbcbsupload67dump_cat']." ";
				...
				<div id="coter_right">
					...
					<ul>
						...
						while ($dataV = $red->fetch()) {
							echo "<li class='cat_n'><a href='categorie_lister.php?sreemsvxy56Y63MIKyvnbsdnsbcbsupload67dump_cat=".$dataV['id_categorie']."'>" .$dataV['nom_categories']. "</a></li>";
						}
						 ?>
					</ul>
					<hr width="250px" /> 
					<form action="tags_lister.php" method="POST">
						<input type="text" name="search_tags" placeholder="Recherche par TAGS" />
						<input type="submit" value="Valider" style="margin-top:0px !important;" class="bout" name="tagsPuts" />
					</form>
				</div>	
		</article>
